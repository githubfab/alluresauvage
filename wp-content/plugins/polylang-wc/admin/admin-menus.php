...
class PLLWC_Admin_Menus {
	...
	public function init() {
		$action = version_compare( WC()->version, '2.7', '<' ) ? 'admin_init' : 'admin_head-nav-menus.php';
		pll_remove_anonymous_object_filter( $action, array( 'WC_Admin_Menus', 'add_nav_menu_meta_boxes' ) );
	...
	public function nav_menu_links() {
		$include = plugin_dir_path( WC_PLUGIN_FILE ) . '/includes/admin/settings/';
		include_once $include . 'class-wc-settings-page.php';
		$settings_account  = include $include . 'class-wc-settings-accounts.php';
		$settings_checkout = include $include . 'class-wc-settings-checkout.php';
		$settings = array_merge( $settings_account->get_settings(), $settings_checkout->get_settings() );

		$i = -1;
		foreach ( $this->get_endpoints() as $key => $value ) {
			$_settings = wp_list_filter( $settings, array( 'default' => $key ) );
