...
class PLLWC_Strings {
	...
	protected function is_translated_email_property( $prop ) {
		return 0 === strpos( $prop, 'subject' ) || 0 === strpos( $prop, 'heading' );
	}
	...
	public function register_strings() {
		...
		foreach ( $this->options as $string => $arr ) {
	...
	public function sanitize_strings( $translation, $name, $context ) {
		if ( 'WooCommerce' === $context ) {
			// Options
			$is_text_field = in_array( $name, wp_list_pluck( $this->options, 'name' ) ) ||
	...
	public function translate_strings() {
		...
		// Options
		foreach ( array_keys( $this->options ) as $string ) {
	...
	public function attribute_label( $label, $name, $product ) {
		...
		if ( $product && ! doing_action( 'wp_ajax_woocommerce_do_ajax_product_export' ) ) {
			$mo = new PLL_MO();
			$mo->import_from_db( PLL()->model->post->get_language( $product->get_id() ) );
