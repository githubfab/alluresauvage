...
class PLLWC_Links_Pro extends PLLWC_Links {
	...
	public function __construct() {
		...
		// Endpoints slugs
		add_action( 'pllwc_email_reload_text_domains', array( $this->slugs_model, 'init_translated_slugs' ) );
	...
	public function edit_address_slugs( $slugs ) {
		foreach ( $slugs as $key => $slug ) {
			if ( isset( $this->slugs_model->translated_slugs[ 'wc_' . $key ] ) && $lang = pll_current_language() ) {
				$slugs[ $key ] = sanitize_title( $this->slugs_model->translated_slugs[ 'wc_' . $key ]['translations'][ $lang ] );
			} else {
				$slugs[ $key ] = sanitize_title( $key ); // Don't rely on woocommerce mo file and accept translation only from our own system
			}
		}
		return $slugs;
	}
