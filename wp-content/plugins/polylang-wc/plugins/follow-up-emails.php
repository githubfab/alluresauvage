...
/**
 * Manages compatibility with Follow-Up Emails
 * Version tested: 4.5.0
 *
 * @since 0.9
 */
class PLLWC_Follow_Up_Emails {
	...
	/**
	 * Synchronize taxonomies associated to Follow-Up emails
	 *
	 * @since 0.9
	 *
	...
	public function copy_post_metas( $keys, $sync, $from ) {
		if ( 'follow_up_email' === get_post_type( $from ) ) {
			// FIXME '_meta', '_product_id', '_category_id' are not copied for now as they need translation
			$to_sync = array(
				'_template',
				'_interval_type',
				'_interval_num',
	...
	/**
	 * Get the email language from email order data
	 *
	 * @since 0.9
	 *
	 * @param array $data
	...
	protected function get_email_order_language( $data ) {
		if ( ! empty( $data['order_id'] ) ) {
			return pll_get_post_language( $data['order_id'] );
	...
	/**
	 * Make sure that emails are sent in the correct language
	 *
	 * @since 0.9
	 *
	 * @param array $data
	...
	public function before_email_send( $subject, $message, $headers, $queue_item ) {
		PLLWC()->emails->before_order_email( $queue_item->email_id ); // Hack as the function usually expects an order id
	}
}
