...
class PLLWC_Bookings {
	...
	public function __construct() {
		...
		add_action( 'pllwc_email_reload_text_domains', array( $this, 'reload_text_domain' ) );
	 ...
	 * @since 0.6
	 */
	public function reload_text_domain() {
		unload_textdomain( 'woocommerce-bookings' );
		$GLOBALS['wc_bookings']->load_plugin_textdomain();
	}
	...
	public function reload_text_domains() {
		// Default text domain for the date
	...
	public function before_booking_metabox_save( $post_id ) {
		if ( isset( $_POST['post_type'], $_POST['wc_bookings_details_meta_box_nonce'] ) && 'wc_booking' === $_POST['post_type'] ) { // Make sure we are on the right page
			...
			if ( $locale !== $booking_locale ) {
				$this->locale = $booking_locale;
	...
	public function new_booking( $booking_id ) {
		...
		$lang = pll_get_post_language( $booking->product_id );
		pll_set_post_language( $booking->id, $lang );
		if ( ! empty( $booking->order_id ) ) {
			pll_set_post_language( $booking->order_id, $lang );
		}
	}
	...
	/**
	 * Copy or synchronize resources and persons
	 *
	 * @since 0.6
	 *
	...
	public function copy_product( $from, $to, $lang, $sync = false ) {
		$this->copy_resources( $from, $to, $lang, $sync );
		$this->copy_persons( $from, $to, $lang, $sync );
		...
		foreach ( $to_translate as $key ) {
			$meta = get_post_meta( $from, $key, true );

			if ( is_array( $meta ) ) {
				// Array keys are post ids and need to be translated
				$tr_meta = array();
				foreach ( $meta as $post_id => $cost ) {
					if ( $tr_id = pll_get_post( $post_id, $lang ) ) {
						$tr_meta[ $tr_id ] = $cost;
					}
				}
				update_post_meta( $to, $key, $tr_meta );
			}
		}
	}
	...
	public function remove_bookable_resource() {
		...
		foreach ( pll_get_post_translations( $post_id ) as $lang => $tr_id ) {
			if ( $tr_id !== $post_id ) { // Let WooCommerce delete the current relationship
	...
	protected function add_metas_to_booking( $post_id, $translations, $meta_key ) {
		...
		if ( ! empty( $query->posts ) ) {
			...
			$wpdb->query( "INSERT INTO {$wpdb->postmeta} ( post_id, meta_key, meta_value ) VALUES " . implode( ',', $values ) ); // WPCS: unprepared SQL ok.
		}
	}
	...
	/**
	 * Translates persons ids _booking_persons meta
	 *
	 * @since 0.6
	 *
	 * @param array  $persons
	...
	public function get_post_metadata( $r, $post_id, $meta_key, $single ) {
		...
		if ( ! $once && $single ) {
			switch ( $meta_key ) {
				...
				case '_booking_persons':
					...
					$value = get_post_meta( $post_id, $meta_key, true );
					$once = false;
					return array( $this->translate_booking_persons_meta( $value, pll_get_post_language( $post_id ) ) ); // WP returns $check[0]
			}
		}
		...
		if ( ! $once && empty( $meta_key ) && 'wc_booking' === get_post_type( $post_id ) ) {
			...
			$value['_booking_persons'] = array( serialize( $this->translate_booking_persons_meta( maybe_unserialize( reset( $value['_booking_persons'] ) ), pll_get_post_language( $post_id ) ) ) );
	 ...
	 * @return array
	 */
	function copy_post_metas( $metas, $sync, $from, $to, $lang ) {
		$to_sync = array(
			'_wc_display_cost',
		...
		$to_copy = array(
			'_wc_booking_resouce_label', // With same typo as in WooCommerce Bookings
		);

		return $sync ? array_merge( $metas, $to_sync ) : array_merge( $metas, $to_sync, $to_copy );
	}
	...
	public function translate_cart_item( $item ) {
		if ( ! empty( $item['booking'] ) ) {
			...
			if ( ! empty( $booking['_resource_id'] ) ) {
				$booking['_resource_id'] = $tr_id = pll_get_post( $booking['_resource_id'] );
			...
			// Translate Duration
			if ( ! empty( $booking['duration'] ) && ! empty( $booking['_duration'] ) ) {
