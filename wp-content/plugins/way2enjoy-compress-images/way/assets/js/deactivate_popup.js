...
function wdReady(prefix){
	...
	jQuery(document).on("change", "[name=" + prefix + "_reasons]", function(){	
		...
		if(jQuery(this).val() == "reason_plugin_is_hard_to_use_technical_problems"){
			...
			additionalInfo = '<div class="wd-additional-active"><div><strong>Please tell the issue you faced.Get 10000 credit free</strong></div><br>' +
			...
			additionalInfo = '<div class="wd-additional-active">' + 
								'<div><strong>Hack me.Change email(Setting->General Settings->email address(verify it) & get 200 free credit unlimited no of times) else try Pro Plan from 0.5$.Consider upgrading.  </strong></div>' +
								'<div><a href=\'https://way2enjoy.com/compress-jpeg?pluginemail='+ window[prefix + "WDDeactivateVars"].email +'\' target="_blank">Upgrade with Paypal.</a>. Get 100000 Credit free if you found plugin which gives better quality</div>';
			...
			additionalInfo = '<div class="wd-additional-active">' + 
								'<div><strong>We have a special offer for you.</strong><a href=\'https://way2enjoy.com/compress-jpeg?pluginemail='+ window[prefix + "WDDeactivateVars"].email +'\' target="_blank">Upgrade with Paypal.</a></div>' +
