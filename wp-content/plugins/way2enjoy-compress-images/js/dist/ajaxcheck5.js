! function() {
    function only_once(fn) {
        var called = !1;
        return function() {
            if (called) throw new Error(wp_way2_msgs.call_bk);
...
(jQuery), jQuery(document).ready(function($) {
    ...
    var data = {
            action: "way2enjoy_requestd"
        },
        errorTpl = '<div class="way2enjoyErrorWrap"><a class="way2enjoyError">' + wp_way2_msgs.failed_h + '</a></div>',
        ...
        requestSuccess = function(data, textStatus, jqXHR) {
            ...
            if (data.html) {
                $button.text(wp_way2_msgs.img_opz);
                var originalSize = (data.type, data.original_size),
                    $originalSizeColumn = $(this).parent().prev("td.original_size");
                $parent.fadeOut("fast", function() {
                    $cell.find(".noSavings, .way2enjoyErrorWrap").remove(), $cell.html(data.html), $cell.find(".way2enjoy-item-details").tipsy(tipsySettings), $originalSizeColumn.html(originalSize), $parent.remove()
                })
            } else if (data.error) {
                var $error = $(errorTpl).attr("title", data.error);
                $parent.closest("td").find(".way2enjoyErrorWrap").remove(), $parent.after($error), $error.tipsy({
                    fade: !0,
                    gravity: "e"
                }), $button.text(wp_way2_msgs.ret_req).removeAttr("disabled").css({
                    opacity: 1
                })
            }
        },
        ...
        requestComplete = function(jqXHR, textStatus, errorThrown) {
            $(this).removeAttr("disabled"), $(this).parent().find(".way2enjoySpinner").css("display", "none")
        },
opts = '<option value="way2enjoy-bulk-lossy">Compress Directory</option>';
...
var getBulkImageDatad = function() {
            ...
            return $rows.each(function() {
                $row = $(this), postId = this.id.replace(/^\D+/g, ""), $row.find("input[type='checkbox'][value='" + postId + "']:checked").length && ($enjoyedBtn = $row.find(".way2enjoy_req"), $enjoyedBtn.length && (btnData = $enjoyedBtn.data(), originalSize = $.trim($row.find("td.original_size").text()), btnData.originalSize = originalSize, rv.push(btnData)))
            }), rv
        },
        bulkModalOptions = {
            zIndex: 4,
        ...
        renderBulkImageSummary = function(bulkImageDatad) {
            var settingd = way2enjoy_settings.api_lossy,
                nImagesd = bulkImageDatad.length,
                headerd = '<p class="way2enjoyBulkHeader">' + wp_way2_msgs.bulkc + ' <span class="close-way2enjoy-bulk">&times;</span></p>',
                enjoyedEmAlld= '<button class="way2enjoy_req_bulk">' + wp_way2_msgs.comp_all + '</button>',
                typeRadiosd = '<div class="radiosWrap"><p>' + wp_way2_msgs.opti_mode + ':</p><label><input type="radio" id="way2enjoy-bulk-type-lossy" value="lossy" name="way2enjoy-bulk-type"/>' + wp_way2_msgs.way2_lossy + '</label>&nbsp;&nbsp;&nbsp;<label><input type="radio" id="way2enjoy-bulk-type-lossless" value="lossless" name="way2enjoy-bulk-type"/>' + wp_way2_msgs.loss_less + '</label></div>',
                $modal = $('<div id="way2enjoy-bulk-modal" class="way2enjoy-modal"></div>').html(headerd).append(typeRadiosd).append('<p class="the-following"><strong>' + nImagesd + '</strong> ' + wp_way2_msgs.opti_mess + ' <a id="rateus" class="button button-primary button-hero" title="' + wp_way2_msgs.rt_us + '" href="https://wordpress.org/support/view/plugin-reviews/way2enjoy-compress-images?rate=5#postform" target="_blank">' + wp_way2_msgs.rt_us + '</a><!--using the <strong class="bulkSetting">' + settingd + "</strong> setting:--></p>").appendTo("body").kmodal(bulkModalOptions).bind($.kmodal.BEFORE_CLOSE, function(event, modal) {}).bind($.kmodal.OPEN, function(event, modal) {}).bind($.kmodal.CLOSE, function(event, modal) {
            ...
            "lossy" === setting1 ? $("#way2enjoy-bulk-type-lossy").attr("checked", !0) : $("#way2enjoy-bulk-type-lossless").attr("checked", !0), $bulkSettingSpan = $(".bulkSetting"), $("input[name='way2enjoy-bulk-type']").change(function() {
                var text = "way2enjoy-bulk-type-lossy" === this.id ? "lossy" : "lossless";
            ...
            var $tabled = $('<table id="way2enjoy-bulkd"></table>'),
                $headerRowd = $('<tr class="way2enjoy-bulk-header"><td>' + wp_way2_msgs.nameuu + '</td><td style="width:120px">' + wp_way2_msgs.original_sz + '</td><td style="width:120px">' + wp_way2_msgs.way2enjoy_st + '</td></tr>');
            $tabled.append($headerRowd), $.each(bulkImageDatad, function(index, element) {
                $tabled.append('<tr class="way2enjoy-item-row" data-way2enjoybulkid="' + element.id + '"><td class="way2enjoy-bulk-filename">' + element.filename + '</td><td class="way2enjoy-originalsize">' + element.originalSize + '</td><td class="way2enjoy-way2enjoyedsize"><span class="way2enjoyBulkSpinner hidden"></span></td></tr>')
            }), $modal.append($tabled).append(enjoyedEmAlld), $(".close-way2enjoy-bulk").click(function() {
                $.kmodal.close()
            }), nImagesd || $(".way2enjoy_req_bulk").attr("disabled", !0).css({
                opacity: .5
            })
        },
        bulkActiond = function(bulkImageDatad) {
            $bulkTabled = $("#way2enjoy-bulkd");
            var parallelprocessd = way2enjoy_settings.bulk_async_limit;
            ...
            var jqxhr = null,
                q = async.queue(function(task, callback) {
                    var id = task.id,
                        $row = (task.filename, $bulkTabled.find("tr[data-way2enjoybulkid='" + id + "']")),
                        $way2enjoyedSizeColumn = $row.find(".way2enjoy-way2enjoyedsize"),
                        $spinner = $way2enjoyedSizeColumn.find(".way2enjoyBulkSpinner").css({
                    ...
                    jqxhr = $.ajax({
                        ...
                        data: {
                            action: "way2enjoy_requestd",
                            id: id,
                            type: $("input[name='way2enjoy-bulk-type']:checked").val(),
                        ...
                        if (data.success && "undefined" == typeof data.message) {
                            ...
                            $way2enjoyedSizeColumn.html(data.html), $way2enjoyedSizeColumn.find(".way2enjoy-item-details").remove(), $savingsPercentColumn.text(savingsPercent), $savingsBytesColumn.text(savingsBytes);
                            var $button = $("button[id='way2enjoyid-" + id + "']"),
                                $parent = $button.parent(),
                                $cell = $button.closest("td"),
                                $originalSizeColumn = $button.parent().prev("td.original_size");
                            $parent.fadeOut("fast", function() {
                                $cell.find(".noSavings, .way2enjoyErrorWrap").remove(), $cell.empty().html(data.html), $cell.find(".way2enjoy-item-details").tipsy(tipsySettings), $originalSizeColumn.html(originalSize), $parent.remove()
                            })
                        } else data.error && wp_way2_msgs.any_fur === data.error && $way2enjoyedSizeColumn.text(wp_way2_msgs.no_svng)
            ...
            q.drain = function() {
                $(".way2enjoy_req_bulk").removeAttr("disabled").css({
                    opacity: 1
                }).text(wp_way2_msgs.doneuu).unbind("click").click(function() {
                    $.kmodal.close()
                })
            }, q.push(bulkImageDatad, function(err) {})
        };
    $btnApplyBulkActiond.add($btnApplyBulkAction2d).click(function(e) {
        if ("way2enjoy-bulk-lossy" === $(this).prev("select").val()) {
            e.preventDefault();
            var bulkImageDatad = getBulkImageDatad();
            renderBulkImageSummary(bulkImageDatad), $(".way2enjoy_req_bulk").click(function(e) {
                e.preventDefault(), $(this).attr("disabled", !0).css({
                    opacity: .5
                }), bulkActiond(bulkImageDatad)
            }), $('.way2enjoy_req_bulk').trigger('click')
        }
    });
    ...
    $("body").on("click", ".way2enjoy-item-details", function(e) {
        var id = $(this).data("id");
        return $(".tipsy").remove(), id == activeId ? (activeId = null, void $(this).text(wp_way2_msgs.shw_dtls)) : ($(".way2enjoy-item-details").text(wp_way2_msgs.shw_dtls), $(this).tipsy("show"), void $(this).text(wp_way2_msgs.hide_dtls))
    }), $("body").on("click", function(e) {
        var $t = $(e.target);
        $t.hasClass("tipsy") || $t.closest(".tipsy").length || $t.hasClass("way2enjoy-item-details") || (activeId = null, $(".way2enjoy-item-details").text("Show details"), $(".tipsy").remove())
        ...
        e.preventDefault();
        var reset = confirm(wp_way2_msgs.all_meta_reset_way2);
        if (reset) {
            var $resetButton = $(this);
            $resetButton.text(wp_way2_msgs.reset_way2_wait).attr("disabled", !0);
            var resetData = {
        ...
        var $button = $(this),
            $parent = $(this).parent();
        data.id = $(this).data("id"), $button.text(wp_way2_msgs.optimizing_img).attr("disabled", !0).css({
            opacity: .5
        }), $parent.find(".way2enjoySpinner").css("display", "inline");
        $.ajax({
            url: ajax_object.ajax_url,
            data: data,
            type: "post",
