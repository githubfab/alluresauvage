...
function($) {
    ...
    var data = {
        ...
        errorTpl = '<div class="way2enjoyErrorWrap"><a class="way2enjoyError1">' + wp_way2_msgs.failed_h + '</a></div>',
        ...
        opts = '<option value="way2enjoy-bulk-lossy">' + wp_way2_msgs.comp_all + '</option>';
    ...
    var getBulkImageData = function() {
        ...
        renderBulkImageSummary = function(bulkImageData) {
            var setting = way2enjoy_settings.api_lossy,
                ...
                header = '<p class="way2enjoyBulkHeader"><strong>' + wp_way2_msgs.bulkc + ' &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; </strong><span class="close-way2enjoy-bulk">&times;</span></p>',
                enjoyedEmAll = '<button class="way2enjoy_req_bulk">' + wp_way2_msgs.comp_all + '</button>',
                typeRadios = '<div class="radiosWrap"><p>' + wp_way2_msgs.opti_mode + ': </p><label><input type="radio" id="way2enjoy-bulk-type-lossy" value="lossy" name="way2enjoy-bulk-type"/>' + wp_way2_msgs.way2_lossy + '</label>&nbsp;&nbsp;&nbsp;<label><input type="radio" id="way2enjoy-bulk-type-lossless" value="lossless" name="way2enjoy-bulk-type"/>' + wp_way2_msgs.loss_less + '</label>',
                $modal = $('<div id="way2enjoy-bulk-modal" class="way2enjoy-modal"></div>').html(header).append(typeRadios).append('<p class="the-following"><strong>' + nImages + '</strong> ' + wp_way2_msgs.thumb_countss + ' ' + wp_way2_msgs.opti_mess + ' </p><span class="way2enjoyBulkHeader1"></span><span class="figures"><a id="rateus" title="' + wp_way2_msgs.rt_us + '" href="https://wordpress.org/support/view/plugin-reviews/way2enjoy-compress-images?rate=5#postform" target="_blank"><span class="likess2"></span></a></span><span class="bulkSetting" style="display:none">' + setting + "</span>").appendTo("body").kmodal(bulkModalOptions).bind($.kmodal.BEFORE_CLOSE, function(event, modal) {}).bind($.kmodal.OPEN, function(event, modal) {}).bind($.kmodal.CLOSE, function(event, modal) {
		...
		//	console.log(setting1);	
            ...
            var $table = $('<table id="way2enjoy-bulk"></table>'),
                $headerRow = $('<tr class="way2enjoy-bulk-header"><td>' + wp_way2_msgs.nameuu + ' <span class="buy2"><a href="https://way2enjoy.com/compress-jpeg?pluginemail=' + admin_email_way2 + '" target="_blank"> ' + wp_way2_msgs.buy_msg + '</a></span></td><td style="width:120px">' + wp_way2_msgs.original_sz + '</td><td style="width:120px">' + wp_way2_msgs.way2enjoy_st + '</td></tr>');
            ...
            }), $modal.append($table).append(enjoyedEmAll), $(".close-way2enjoy-bulk").click(function() {
                $.kmodal.close()
            }), nImages || $(".way2enjoy_req_bulk").attr("disabled", !0).css({
                opacity: .5
            })
        },
        bulkAction = function(bulkImageData) {
            $bulkTable = $("#way2enjoy-bulk");
            var parallelprocess = way2enjoy_settings.bulk_async_limit;
            ...
            var jqxhr = null,
                q = async.queue(function(task, callback) {
                    var id = task.id,
                        $row = (task.filename, $bulkTable.find("tr[data-way2enjoybulkid='" + id + "']")),
                    ...
                    jqxhr = $.ajax({
                        ...
                        if (data.success && "undefined" == typeof data.message) {
                            ...
                            $way2enjoyedSizeColumn.html(data.html), $way2enjoyedSizeColumn.find(".way2enjoy-item-details").tipsy(tipsySettings) /*,$way2enjoyedSizeColumn.find(".way2enjoy-item-details").remove()*/ , $savingsPercentColumn.text(savingsPercent), $savingsBytesColumn.text(savingsBytes);
                            var $button = $("button[id='way2enjoyid-" + id + "']"),
                                $parent = $button.parent(),
                                $cell = $button.closest("td"),
                                $originalSizeColumn = $button.parent().prev("td.original_size"); 
								if(quotaRema===undefined){var ramukaka='' + wp_way2_msgs.buy_msg + '';}else{var ramukaka=''+quotaRema+'  ' + wp_way2_msgs.quotabal_dtls + '';	}								
		$(".way2enjoyBulkHeader1").text(ramukaka);	
    ...
    $btnApplyBulkAction.add($btnApplyBulkAction2).click(function(e) {
        if ("way2enjoy-bulk-lossy" === $(this).prev("select").val()) {
            ...
            renderBulkImageSummary(bulkImageData), $(".way2enjoy_req_bulk").click(function(e) {
    ...
    $("body").on("click", ".way2enjoy-item-details", function(e) {
        ...
        var $t = $(e.target);
        $t.hasClass("tipsy") || $t.closest(".tipsy").length || $t.hasClass("way2enjoy-item-details") || (activeId = null, $(".way2enjoy-item-details").text(wp_way2_msgs.shw_dtls), $(".tipsy").remove())
